...
class ShortestDepenencyPath:
    ...
    """
        从两个实体的节点出发，递归地寻找他们的父亲节点，
        若两个父亲节点的集合存在交集，则最短依存树搜索完成
    """
    def get_shortest_dependent_path(self, word1, word1_index, word2, word2_index, tdl, is_first):
        if is_first == True:  # 第一次运行的时候把当前词也放到，Word1_father中
            ...
            self.word2_father_word.append(word2)
        ...
        for i in range(len(self.word2_father_word)):  # word1的父节点一次与word2_father中节点比较是否相同
            if word1_father_id == self.word2_father_index[i]:  # 存在依存子数， 将树的两个分支给出
                ...
                for h in range(len(self.word1_father_index)):
                    if self.word1_father_index[h] == self.word2_father_index[i]:  # word1_father_index序列中第几个与word2_father_index.get(i)相等
                        record_loc = h
                ...
                for k in range(record_loc+1):
                    ...
                    if k == record_loc:
                        ...
                        branch1 += self.word1_father_word[k]+"_"+str(self.word1_father_index[k])+"__("+self.word1_father_rela[k]+")__"
                ...
                for e in range(i+1):
                    ...
                    if e == i:
                        ...
                        branch2 += self.word2_father_word[e]+"_"+str(self.word2_father_index[e])+"__("+self.word2_father_rela[e]+")__"
            ...
            if word2_father_id == self.word1_father_index[j]: # 存在依存书
                ...
                for l in range(record_loc+1):
                    ...
                    if l == record_loc:
                        ...
                        branch4 += self.word2_father_word[l]+"_"+str(self.word2_father_index[l])+"__("+self.word2_father_rela[l]+")__"
                ...
                for e in range(j+1):
                    ...
                    if e == j:
                        ...
                        branch3 += self.word1_father_word[e]+"_"+str(self.word1_father_index[e])+"__("+self.word1_father_rela[e]+")__"


        sdp_path = ""
        if mark1 == False and mark2 == False:  # 没有找到最短依存路径
            # 如果没有相同的，则继续找两个父节点的子节点
            sdp_path = self.get_shortest_dependent_path(word1_father_value, word1_father_id, word2_father_value, word2_father_id, tdl, False)
    ...
    def get_node_father(self, word_id, tdl, father_node_word, father_node_index, rela_set):
        ...
        father_index = -1
        for tdp in tdl:
            if word_id == tdp.dep_index:  # 通过关系中的子节点找父节点
                # 等价与判断是否为ROOT节点
